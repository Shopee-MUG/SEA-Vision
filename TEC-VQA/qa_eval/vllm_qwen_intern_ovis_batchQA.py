...
try:
    from qwen_vl_utils import process_vision_info  # Optional but preferred for Qwen-family formatting.
except Exception:
...
class QADataset(Dataset):
    def __init__(self, qa_data, image_base_dir, processor, system_prompt, model_name, use_en=False):
    ...
    def __getitem__(self, idx):
        ...
        if "<|image_pad|>" in prompt_text and ("internvl" in self.model_name or "ovis" in self.model_name):
            prompt_text = prompt_text.replace("<|image_pad|>", "<image>")
        ...
        if process_vision_info is not None:
            try:
                ...
                mm_data["image"] = Image.open(image_path).convert("RGB")
        else:
            mm_data["image"] = Image.open(image_path).convert("RGB")
...
def parse_args():
    ...
    parser.add_argument("--processor_dir", type=str, default="", help="Optional processor path")
...
if __name__ == "__main__":
    ...
    processor_source = args.processor_dir if args.processor_dir else model_dir
